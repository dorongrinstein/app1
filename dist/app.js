...
$('run').addEventListener('click', () => {
  ...
  try {
    ...
    timeout = setTimeout(() => { stopWorker(); setBusy(false); status('The operation took too long. Try again on a device with more available memory.', true); }, 120000);
    ...
    worker.onerror = event => {
      event.preventDefault(); stopWorker(); setBusy(false);
      status('Encryption could not start. Reload the app in a current browser and check available memory.', true);
    };
    worker.postMessage({ mode, text: importedText ?? $('source').value, password: $('password').value });
  } catch { stopWorker(); setBusy(false); status('Encryption is unavailable in this browser. Try a current browser over HTTPS.', true); }
});
